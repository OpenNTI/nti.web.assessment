...
const t = scoped('nti-assessment.survey.editor.parts.Availability', {
	...
	disabledDescription: 'To protect students\' work, certain editing functionality has been disabled. To re-enable those features an instructor must reset submissions and unpublish the survey.',
...
export default function SurveyAvailability () {
	...
	const canResetAndUnpublish = survey.hasLink('Reset') || survey.canUnpublish();
	...
	return (
		<div className={cx('survey-availability')}>
			<div className={cx('disclaimer')}>
				...
				<Text.Base className={cx('description')}>{canResetAndUnpublish ? t('description') : t('disabledDescription')}</Text.Base>
