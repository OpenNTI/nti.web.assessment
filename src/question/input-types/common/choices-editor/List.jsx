...
const orderReducer = (state, action) => {
	switch (action.type) {
		case Actions.reset:
			return {
				order: action.choices.map((c, index) => index),
				choices: action.choices,
			};
		case Actions.change:
			return {
				order: arr.move(state.order, action.original, action.updated),
...
export default function ChoiceList({
	...
	const onChoiceChange = (index, choice) =>
		onChange([
	...
	const onChoiceRemove = useCallback(
		index => (
			onChange?.([
				...choices.slice(0, index),
				...choices.slice(index + 1),
